...
aes_key_wrap(aes_key_wrap_ctx *ctx, const u_int8_t *P, size_t n, u_int8_t *C)
{
	...
	bcopy(P, C + 8, n * 8);	/* P and C may overlap */
	A = C;				/* A points to C[0] */
	...
	for (j = 0, t = 1; j <= 5; j++) {
		...
		for (i = 1; i <= n; i++, t++) {
			...
			memcpy(&B[1], R, 8);
			/* B = AES(K, B) */
			rijndael_encrypt(&ctx->ctx, (u_char*)B, (u_char*)B);
			/* MSB(64, B) = MSB(64, B) ^ t */
			B[0] ^= htobe64(t);
			/* A = MSB(64, B) */
			memcpy(A, &B[0], 8);
...
aes_key_unwrap(aes_key_wrap_ctx *ctx, const u_int8_t *C, u_int8_t *P, size_t n)
{
	...
	memcpy(A, C, 8);		/* A = C[0] */
	bcopy(C + 8, P, n * 8);	/* P and C may overlap */
	...
	for (j = 5, t = 6 * n; j >= 0; j--) {
		...
		for (i = n; i >= 1; i--, t--) {
			...
			memcpy(&B[1], R, 8);
			/* B = AES-1(K, B) */
			rijndael_decrypt(&ctx->ctx, (u_char*)B, (u_char*)B);
			/* A = MSB(64, B) */
			memcpy(A, &B[0], 8);
			/* R[i] = LSB(64, B) */
			memcpy(R, &B[1], 8);
